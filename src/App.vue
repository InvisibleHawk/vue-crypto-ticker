  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="container">
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="ticker" type="text" name="wallet" id="wallet" @input="change" @keydown.enter="addTicker"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Например DOGE" />
            </div>
            <div class="flex bg-white shadow-md p-1 rounded-md flex-wrap">
              <span @click="getPresetCoin" v-for="(coin, idx) in coinsHint" :key="idx"
        ...
        <button @click="reset"
          class="border block text-white bg-gray-600 rounded-full font-medium px-3 py-1">Сброс</button>
      </section>
      ...
      <template v-if="tickers.length">
        ...
        <div class="mt-1 w-40 relative rounded-md shadow-md">
          <input v-model="filter" @input="page = 1" type="text" name="filter" id="filter"
            class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
            placeholder="Например DOGE" />
        </div>
        <button v-if="page > 1" @click="page = page - 1" type="button"
          class="my-4 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
          Назад
        </button>
        <button v-if="hasNextPage" @click="page = +page + 1" type="button"
...
export default {
  ...
  async created() {
    ...
    const tickerData = localStorage.getItem('coin-list')
  ...
  computed: {
    coinsHint() {
      if (!this.ticker) return ['BTC', 'ETH', 'DOGE', 'CHD', 'BTH']

      const normTicker = this.ticker.toUpperCase()
      let coins = this.coinsList.filter(coin => coin.includes(normTicker))
  ...
  methods: {
    ...
    addTicker() {
      ...
      this.isAdded = false
      const foundTicker = this.tickers.find(tickerItem => this.ticker === tickerItem.name)
      if (foundTicker) {
        this.isAdded = true
        return
      }
